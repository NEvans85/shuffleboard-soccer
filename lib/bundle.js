...
class Environment {
  constructor() {
    ...
    this.movingEntities[0].applyForce([100, 100]);
  }
...
class Entity {
  ...
  applyKineticFriction() {
    [this.velX, this.velY].forEach(velocity => {
      velocity *= this.frictionCoefficient;
      if (velocity < 1) {
        velocity = 0;
      }
    });

    // The follow code is too complicated and doesn;t work right.
    // let forceX = 0;
    // let forceY = 0;
    // const force = this.frictionCoefficient * this.gravity * this.mass;
    // if (this.velX !== 0 && this.velY !== 0) {
