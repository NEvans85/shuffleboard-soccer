...
environment.pucks[3].setVelocity([0, 0]);
environment.pucks[4].setVelocity([0, 300]);
...
const nextFrame = () => {
  ...
  collisions.forEach(collidingEntities => {
    ...
    Object(__WEBPACK_IMPORTED_MODULE_3__collision_resolver__["a" /* resolveCollision */])(collidingEntities[0], collidingEntities[1]);
  });
  // console.log(environment.pucks[4]);
...
const resolveCirCirCollision = (circle1, circle2) => {
  ...
  console.log(`c1VelMag = ${c1VelMag}`);
  const c2VelMag = pythagoreanResult(circle2.velX, circle2.velY);
  console.log(`c2VelMag = ${c2VelMag}`);
...
const findAngleOfCollision = (circle1, poc) => {
  ...
  if (poc[1] === circle1.posY) {
    ...
    aoc = Math.atan((poc[0] - circle1.posX) / (poc[1] - circle1.posY));
  }
  if (aoc < 0) {
    aoc += 2 * Math.PI;
  }
